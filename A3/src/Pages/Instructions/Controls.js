...
import { ButtonNext, ButtonLast, ButtonBack, WithStore } from 'pure-react-carousel';
...
export default function Controls({ final }) {
  let buttons = (
      <>
          ...
          <Button as={ButtonBack} class="ui circular icon button" icon="chevron left" />
          <Button as={ButtonNext} class="ui circular icon button" style={{marginRight:20}} icon="chevron right" />
      </>
  );
  if (final) {
      buttons = (
          <>
              <Button as={ButtonBack} class="ui circular icon button"  icon="chevron left" />
              <Button class="ui orange-btn button" style={{marginRight:20}} >Ready to play!</Button> 
          </>
      );
  }
  return (
